...
?>
<!-- Modal-proveedores --------------------------------------------------------------------------------------------------------------------------------->
<div class="modal" id="adj2" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
...
<form action="../modal/procesos/carga2.php" method="post" enctype="multipart/form-data">
...
<select name="guia" id="guia" placeholder="Seleccione la guia" class="form-control" type="text" required data-error="Seleccione la solicitud">
<option value="">Selecionar solicitud</option>                            
<?php
          $query = ("SELECT * FROM guia");
          $recupera=mysqli_query($conn, $query);
          while ($nguia = mysqli_fetch_array($recupera)) {
          echo '<option value="'.$nguia[nguia].'">'.$nguia[nguia].'</option>';
          }
?>
<center>
<div class="col-sm-6">
